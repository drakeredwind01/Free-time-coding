    ...
    def main(self, screen):
        ...
        while True:
            ...
            for n in range(0, screen_size[0], stepsize + 1):
                ...
                try:
                    inputnote = round(freq_from_autocorr(raw_data_signal, SR.RATE),
                                      2)  #### find the freq from the audio sample
                ...
                targetnote = closest_value_index(frequencies,
                                                 round(inputnote, 2))  #### find the closest note in the keyed array
                position = ((n), (screen_size[1] - (int(
                    screen_size[1] / (frequencies[top_note] - frequencies[bot_note]) * (
                                inputnote - frequencies[bot_note])))))
                ...
                if needle:
                    pygame.draw.line(screen, stepsizecolor, centrescreen, (
                    centrescreen[0] + ((frequencies[targetnote] - inputnote) * 20), centrescreen[1] - 200), 3)
                ...
                err = abs(frequencies[targetnote] - inputnote)
                text2 = font2.render(
                    "Range: " + str(tunerNotes[frequencies[bot_note]]) + " - " + str(tunerNotes[frequencies[top_note]])
                    + "    Show Notes(n):" + str(shownotes) + "    Circles(c):" + str(circ)
                    + "    Lines(l):" + str(line) + "    Stepsize(v<>b):" + str(stepsize)
                    + "    Fill Circles(f):" + str(not fill) + "    Rand Clr(r):" + str(
                        col) + "    Clr Screen(m):" + str(cls) + "   Lev: " + meter[1:int(20 - signal_level)]
                    , 1, (255, 255, 0))
